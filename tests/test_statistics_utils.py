...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_basic_case(self):
        ...
        - Verificar que el resultado es correcto (ej: [1.5, 2.5, 3.5] para el array dado) (usar numpy.testing.assert_allclose() 
          comparar arrays de NumPy - esto es mejor que unittest porque maneja la comparación de arrays numéricos con tolerancia
            para errores de punto flotante)
        - Verificar que el resultado tiene la forma (shape) esperada (usar self.assertEqual para comparar 
        tuplas de .shape - comparación simple, unittest es suficiente)
        """
        utils = StatisticsUtils()
        arr1 = [1, 2, 3, 4]
        result = utils.moving_average(arr1, window=2)
    ...
    def test_moving_average_raises_for_invalid_window(self):
        ...
        - Llamar a moving_average con window mayor que la longitud del array y verificar que se lanza un ValueError 
        (usar self.assertRaises)
        """
        utils = StatisticsUtils()
        arr2 = [1, 2, 3]
        with self.assertRaises(ValueError):
            utils.moving_average(arr2, window=0) #con window=0
        with self.assertRaises(ValueError):
            utils.moving_average(arr2, window=5) #window mayor que la longitud del array
    ...
    def test_moving_average_only_accepts_1d_sequences(self):
        ...
        - Llamar a moving_average con esa secuencia y verificar que se lanza un ValueError indicando que solo se 
        aceptan secuencias 1D (usar self.assertRaises)
        """
        utils = StatisticsUtils()
        arr3 = [[1, 2], [3, 4]]
        with self.assertRaises(ValueError):
            utils.moving_average(arr3, window=2)
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        - Verificar que la media del resultado es aproximadamente 0 (usar self.assertAlmostEqual para un solo valor 
    ...
    def test_zscore_raises_for_zero_std(self):
        ...
        - Llamar a zscore con esa secuencia y verificar que se lanza un ValueError indicando que la desviación estándar es cero
          (usar self.assertRaises)
        """
        utils = StatisticsUtils()
        arr5 = [5, 5, 5]
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        - Verificar que los valores transformados son correctos (ej: [0.0, 0.5, 1.0] para [2, 4, 6])
          (usar numpy.testing.assert_allclose() para comparar el array completo - esto es necesario para comparar arrays de NumPy 
          con tolerancia para errores de punto flotante)
        """
        
        utils = StatisticsUtils()
        arr6 = [2, 4, 6]
        result = utils.min_max_scale(arr6)
        self.assertAlmostEqual(result.min(), 0.0, places=5)
        self.assertAlmostEqual(result.max(), 1.0, places=5)
        np.testing.assert_allclose(result, [0.0, 0.5, 1.0], rtol=1e-5)
    ...
    def test_min_max_scale_raises_for_constant_values(self):
        ...
        - Llamar a min_max_scale con esa secuencia y verificar que se lanza un ValueError indicando que todos los valores son iguales
          (usar self.assertRaises)
        """
        utils = StatisticsUtils()
        arr7 = [3, 3, 3]
        with self.assertRaises(ValueError):
            utils.min_max_scale(arr7)
